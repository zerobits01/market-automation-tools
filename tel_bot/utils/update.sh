...
then
	do_changes=1; echo date; date > /tmp/update_changes.log; [[ $(cd /home/zerobits01/time-clone; git pull)=="Already up to date." ]] || do_changes=0 
        [[ $do_changes -eq 0 ]] && echo install; npm install >> /tmp/update_changes.log && echo build; npm run build >> /tmp/update_changes.log && echo restart; sudo systemctl restart kit365-ui.service >> /tmp/update_changes.log
